...
menu_v2ray() {
    ...
    # === DETECTAR DISPOSITIVOS POR IP (CORREGIDO) ===
    get_devices() {
        local name=$1
        local now=$(date +%s)
        local -A ips
        ...
        if [ -f "$LOG_DIR/access.log" ]; then
            while IFS= read -r line; do
                if [[ "$line" == *"accepted"* && "$line" == *"$name"* ]]; then
                    local ip=$(echo "$line" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -1)
                    local ts=$(echo "$line" | awk '{print $1" "$2}' | xargs -I{} date -d "{}" +%s 2>/dev/null || echo 0)
                    if (( now - ts < 300 )) && [[ -n "$ip" ]]; then
                        ips["$ip"]=1
                    fi
                fi
            done < <(tac "$LOG_DIR/access.log" | grep -m 100 "accepted")
        fi
    ...
    install_xray() {
        ...
        # === CRON CADA MINUTO (NOHUP + CRONTAB) ===
        (crontab -l 2>/dev/null | grep -v "update_and_get_stats"; echo "* * * * * /bin/bash -c 'source /root/scrip.sh; update_and_get_stats'") | crontab -
        nohup bash -c 'while true; do source /root/scrip.sh; update_and_get_stats; sleep 60; done' >/dev/null 2>&1 &
    }
...
EOF
    ...
    # === ACTUALIZAR ESTADÍSTICAS CADA MINUTO (CORREGIDO) ===
    update_and_get_stats() {
        local now=$(date +%s)
        local stats_output=$($XRAY_BIN api statsquery --server=127.0.0.1:$API_PORT 2>/dev/null | grep -E '"name"|value' || echo "")
        [[ -z "$stats_output" ]] && return 1

        local temp_stats=$(mktemp)
        echo "{" > "$temp_stats"
        echo "$stats_output" | paste -d '' - - | sed 's/"name": "\([^"]*\)", "value": \([^}]*\)/"\1": \2,/g' >> "$temp_stats"
        sed -i '$ s/,$/}/' "$temp_stats"
        ...
        while IFS=: read -r name total_up total_down total_time last_check last_up last_down session_start last_activity session_up session_down; do
            ...
            if (( diff_up > 0 || diff_down > 0 )); then
                if (( last_activity == 0 )); then
                    ...
                    new_session_up=$((session_up + diff_up))
                    new_session_down=$((session_down + diff_down))
                    new_last_activity=$now
                fi
                local time_since=$(( (now - last_check) / 60 ))
                (( time_since > 0 )) && new_total_time=$((total_time + time_since))
            fi
    ...
    # === VER ONLINE Y ESTADÍSTICAS (FORMATO CORREGIDO) ===
    view_online_and_stats() {
        ...
        while IFS=: read -r name total_up total_down total_time last_check last_up last_down session_start last_activity session_up session_down; do
            ...
            echo -e "${DATA} ${WHITE}Transferencia total:${NC} ${CYAN}${total_transfer_gb} GB (↑ ${up_gb} GB | ↓ ${down_gb} GB)${NC}"
    ...
    # === RESTO DE FUNCIONES (add_user, remove_user_menu, list_users, etc.) ===
    # [Aquí van todas las funciones que ya tenías: add_user, remove_user_menu, list_users, export_all_vmess, backup_v2ray, restore_v2ray, etc.]
    # → Las dejo igual, solo copié las partes críticas arriba.

    # === MENÚ PRINCIPAL (show_v2ray_menu) ===
    show_v2ray_menu() {
        reset_terminal
        while true; do
            reset_terminal
            if [ ! -f "$SERVER_ADDR_FILE" ]; then
                read -p "Ingrese IP o dominio: " addr
                [[ -z "$addr" ]] && addr="IP_NO_DETECTADA"
            ...
            echo -e " ${STAR}12) ${CYAN}Ver online, tiempo y transferencia${NC}"
            echo -e " ${STAR} 0) ${GRAY}Volver${NC}"
            echo -e "${PURPLE}════════════════════════════════════════════════${NC}"
            read -p " ${ROCKET} Elige: " opt

            case $opt in
                1) install_xray; read -p "IP: " s; [[ -n "$s" ]] && echo "$s" > "$SERVER_ADDR_FILE"; read -p "Path: " p; read -p "Host: " h; generate_config "$p" "$h"; create_service; systemctl restart xray; read -p "Enter..." -r </dev/tty;;
                2) read -p "IP: " s; [[ -n "$s" ]] && echo "$s" > "$SERVER_ADDR_FILE"; read -p "Path: " p; read -p "Host: " h; generate_config "$p" "$h"; systemctl restart xray; read -p "Enter..." -r </dev/tty;;
                ...
                8) systemctl stop xray; systemctl disable xray; rm -f "$SERVICE_FILE" "$XRAY_BIN"; rm -rf "$CONFIG_DIR" "$LOG_DIR" "$BACKUP_DIR"; echo -e "${CHECK} Desinstalado."; return;;
