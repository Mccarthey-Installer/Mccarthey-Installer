...
if [[ -t 0 ]]; then
    while true; do
        ...
        # Bucle para ignorar Enter vacío + soporte para flechas, borrar, historial
        while :; do
            # -e = habilita edición con flechas, backspace, etc.
            if read -e -p "$PROMPT" OPCION; then
                [[ -z "$OPCION" ]] && continue  # Si solo Enter → repetir
                break
            else
                # Si EOF (Ctrl+D), salir
                echo -e "\n${AMARILLO_SUAVE}🚪 Saliendo...${NC}"
                exit 0
            fi
        done
        ...
        case "$OPCION" in
            ...
            *) 
                echo -e "${ROJO}❌ ¡Opción inválida!${NC}"
                read -e -p "$(echo -e ${ROSA_CLARO}Presiona Enter para continuar...${NC})"
                ;;
        esac
    done
fi
