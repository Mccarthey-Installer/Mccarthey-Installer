                                ...
                                case \"\$MSG_TEXT\" in
                                    ...
                                    '6')
                                        if [[ ! -f \"\$REGISTROS\" || ! -s \"\$REGISTROS\" ]]; then
                                            ...
                                            curl -s -X POST \"\$URL/sendDocu ment\" -F chat_id=\$CHAT_ID -F document=@\"\$temp_backup\" -F caption=\"💾 *Aquí está tu backup de usuarios.*\" -F parse_mode=Markdown >/dev/null
                                            rm -f \"\$temp_backup\"
                                        fi
                                        ;;
                                    '7')
