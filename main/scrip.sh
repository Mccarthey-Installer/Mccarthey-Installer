...
EOF
    ...
    echo -e "\033[1;36m🌟 USUARIOS ONLINE Y ESTADÍSTICAS 🌟\033[0m"
    echo -e "════════════════════════════════════\n"

    local user_stats=$(cat /root/stats.json | jq -r '.[] | [.name, .online, .last_seen, .transfer, .session_time, .total_time, .devices] | @tsv')
    local total_users=0
    local active_users=0

    while IFS=$'\t' read -r name online last_seen transfer session_time total_time devices; do
        ((total_users++))
        if [[ "$online" == "true" ]]; then
            ((active_users++))
            echo -e "🟢 \033[1;32m${name}\033[0m"
            echo -e "🔑 Online: Sí (${devices} móviles) 🚀"
            echo -e "⏱️ Sesión: ${session_time}"
            echo -e "📊 Transferencia: ${transfer}"
            echo -e "⏱️ Total conectado: ${total_time}"
            echo -e "────────────────────────────────────\n"
        else
            echo -e "🔴 \033[1;31m${name}\033[0m"
            echo -e "🔑 Estado: No ─ Última: ${last_seen}"
            echo -e "📊 Transferencia: ${transfer}"
            echo -e "⏱️ Total conectado: ${total_time}"
            echo -e "────────────────────────────────────\n"
        fi
    ...
    echo -e "📋 Resumen:"
    echo -e "👥 Total usuarios: ${total_users}"
    echo -e "🟢 Activos: ${active_users}"
    echo -e "🔴 Inactivos: $((total_users - active_users))"
