...
systemctl restart sshd && echo "SSH configurado correctamente."
    ...
    echo -e "${AMARILLO_SUAVE}3. 🔄 Reiniciar Bot${NC}"
...
🔢 O usa: 1, 2, 3, 4, 5, 6, 7, 0\" -d parse_mode=Markdown >/dev/null
        ...
        3)
            if [[ -f "$PIDFILE" ]]; then
                ...
                rm -f "$PIDFILE"
                pkill -f "api.telegram.org"
                echo -e "${AMARILLO_SUAVE}🔄 Bot detenido. Reiniciando...${NC}"
            else
                echo -e "${AMARILLO_SUAVE}🔄 No hay bot corriendo. Activando...${NC}"
            fi
            # Reutilizar el código de activación sin pedir inputs de nuevo (asumiendo que los archivos existen)
            if [[ -f /root/sshbot_token && -f /root/sshbot_userid && -f /root/sshbot_username ]]; then
...
🔢 O usa: 1, 2, 3, 4, 5, 6, 7, 0\" -d parse_mode=Markdown >/dev/null
            ...
            " >/dev/null 2>&1 &
                echo $! > "$PIDFILE"
                echo -e "${VERDE}✅ Bot reiniciado y corriendo en segundo plano (PID: $(cat $PIDFILE)).${NC}"
                echo -e "${AMARILLO_SUAVE}💡 El bot responderá a 'hola' con el menú interactivo.${NC}"
            else
                echo -e "${ROJO}❌ No se encontraron credenciales del bot. Usa la opción 1 para activar primero.${NC}"
            fi
            ;;
        0)
            return
